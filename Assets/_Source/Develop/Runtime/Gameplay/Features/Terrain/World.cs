...
public class World : MonoBehaviour
{
    ...
    [System.NonSerialized] public Dictionary<Vector2Int, ChunkData> chunks = new();
    ...
    public bool TryGetBlock(Vector3Int worldPos, out BlockState blockState)
    {
        Vector2Int chunkPos = GetChunkCoordinatesContainingBlock(worldPos);
        blockState = null;

        if (!chunks.TryGetValue(chunkPos, out ChunkData chunkData)) return false;

        Vector3Int localPos = worldPos - new Vector3Int(chunkPos.x, 0, chunkPos.y) * ChunkWidth;
        if (localPos.x < 0 || localPos.x >= ChunkWidth || localPos.y < 0 || localPos.y >= ChunkHeight || localPos.z < 0 || localPos.z >= ChunkWidth) return false;

        blockState = chunkData.Blocks[localPos.x, localPos.y, localPos.z];
        return true;
    }

    public bool TrySetBlock(Vector3Int worldPos, BlockState blockState)
    {
        Vector2Int chunkPos = GetChunkCoordinatesContainingBlock(worldPos);
        if (!chunks.TryGetValue(chunkPos, out ChunkData chunkData)) return false;
        ...
        chunkData.Blocks[localPos.x, localPos.y, localPos.z] = blockState;
        MarkChunkAndBoundaryNeighborsDirty(chunkPos, localPos);
        return true;
    }
