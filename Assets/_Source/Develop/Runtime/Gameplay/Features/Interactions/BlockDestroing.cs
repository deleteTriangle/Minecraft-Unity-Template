...
public class BlockDestroing : MonoBehaviour
{
    ...
    {
        if (!G.World.TryGetBlock(pos, out BlockState blockState)) return;

        if (blockState.type == BlockType.Door)
        {
            Vector3Int otherPos = BlockStateFlags.IsUpper(blockState) ? pos + Vector3Int.down : pos + Vector3Int.up;
            G.World.TrySetBlock(otherPos, new BlockState(BlockType.Air));
        }

        if (!G.World.TrySetBlock(pos, new BlockState(BlockType.Air))) return;

        OnBlockDestroyed?.Invoke(G.World.GetChunkCoordinatesContainingBlock(pos));
    }

    private BlockState GetBlockAtPos(Vector3Int pos)
    {
        return G.World.TryGetBlock(pos, out BlockState blockState) ? blockState : null;
    }
}
